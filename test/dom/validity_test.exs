defmodule DOM.ValidityTest do
  ...
  describe "participation" do
    test "a non-form element matches neither :valid nor :invalid" do
      ...
      refute DOM.Element.matches(q(doc, "#d"), ":valid")
      refute DOM.Element.matches(q(doc, "#d"), ":invalid")
    end

    test "an unconstrained input is :valid" do
      doc = new_document("<body><input id='i'></body>")
      assert DOM.Element.matches(q(doc, "#i"), ":valid")
      refute DOM.Element.matches(q(doc, "#i"), ":invalid")
    end
  ...
  describe ":invalid sources" do
    test "required + empty is :invalid; required + filled is :valid" do
      ...
      assert DOM.Element.matches(q(doc, "#a"), ":invalid")
      refute DOM.Element.matches(q(doc, "#b"), ":invalid")
      assert DOM.Element.matches(q(doc, "#b"), ":valid")
    end
    ...
    test "type=email with a bad value is :invalid; good is :valid" do
      ...
      assert DOM.Element.matches(q(doc, "#e"), ":invalid")
      assert DOM.Element.matches(q(doc, "#e2"), ":valid")
    end
    ...
    test "pattern mismatch is :invalid; match is :valid" do
      ...
      assert DOM.Element.matches(q(doc, "#p"), ":invalid")
      assert DOM.Element.matches(q(doc, "#p2"), ":valid")
    end
  end
  ...
  describe ":in-range / :out-of-range" do
    ...
    test "a value below min or above max is :out-of-range and :invalid", %{doc: doc} do
      assert DOM.Element.matches(q(doc, "#lo"), ":out-of-range")
      assert DOM.Element.matches(q(doc, "#lo"), ":invalid")
      assert DOM.Element.matches(q(doc, "#hi"), ":out-of-range")
    end

    test "a value within min..max is :in-range and :valid", %{doc: doc} do
      assert DOM.Element.matches(q(doc, "#ok"), ":in-range")
      assert DOM.Element.matches(q(doc, "#ok"), ":valid")
      refute DOM.Element.matches(q(doc, "#ok"), ":out-of-range")
    end

    test "a non-range input matches neither :in-range nor :out-of-range", %{doc: doc} do
      refute DOM.Element.matches(q(doc, "#plain"), ":in-range")
      refute DOM.Element.matches(q(doc, "#plain"), ":out-of-range")
    end
  end
end
