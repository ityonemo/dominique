defmodule DOM.TargetTest do
  ...
  test "no fragment: nothing matches :target" do
    ...
    refute DOM.Element.matches(DOM.query_selector(doc, "#sec"), ":target")
  end
  ...
  test "set_fragment makes the id-matching element :target" do
    ...
    assert DOM.Element.matches(DOM.query_selector(doc, "#a"), ":target")
    refute DOM.Element.matches(DOM.query_selector(doc, "#b"), ":target")
  end
  ...
  test "changing the fragment moves :target" do
    ...
    refute DOM.Element.matches(DOM.query_selector(doc, "#a"), ":target")
    assert DOM.Element.matches(DOM.query_selector(doc, "#b"), ":target")
  end
  ...
  test ":target is case-sensitive" do
    ...
    refute DOM.Element.matches(DOM.query_selector(doc, "#X"), ":target")
  end
  ...
  test "id wins over a[name] for the same fragment" do
    ...
    assert DOM.Element.matches(DOM.query_selector(doc, "#x"), ":target")
    refute DOM.Element.matches(DOM.query_selector(doc, "a"), ":target")
  end
end
